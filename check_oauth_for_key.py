#!/usr/bin/env python3
"""
Test to see if Qwen OAuth service provides DashScope API keys
"""
...
async def check_qwen_oauth_for_dashscope_key():
    """Check if Qwen OAuth service provides DashScope API keys."""
    print("Checking if Qwen OAuth service provides DashScope API keys...")
    ...
    # Try Qwen OAuth endpoints that might provide API keys
    oauth_endpoints = [
        "https://chat.qwen.ai/api/v1/user/apikeys",
        "https://chat.qwen.ai/api/v1/apikeys",
        "https://chat.qwen.ai/api/v1/dashscope/apikeys",
        "https://chat.qwen.ai/api/v1/user/dashscope/key"
    ]
                    ...
                    if response.status == 200:
                        try:
                            ...
                            if response_text.strip().startswith('<!DOCTYPE') or '<html' in response_text:
                                print("Response is HTML, not API response")
                            else:
...
if __name__ == "__main__":
    asyncio.run(check_qwen_oauth_for_dashscope_key())